...
from arxiv_auth.auth import tokens, scopes
from pytz import UTC

from arxiv.base.globals import get_application_config

from src.arxiv.submission import Agent, User
...
def get_system_token(name: str, agent: Agent, scopes: List[str]) -> str:
    ...
    return str(tokens.encode(session, secret))


def get_compiler_scopes(resource: str) -> List[str]:
    """Get minimal auth scopes necessary for compilation integration."""
    return [scopes.READ_COMPILE.for_resource(resource),
            scopes.CREATE_COMPILE.for_resource(resource)]
