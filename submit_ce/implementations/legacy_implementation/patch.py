...
Legacy changes to the paper are represented as NG events that are patches in to
the event history.
...
def patch_hold(submission: domain.Submission,
    ...
    if not (row.type == Submission.NEW_SUBMISSION or row.type == Submission.REPLACMENT):
        raise ValueError('Only applies to new and replacement rows')

    if row.status == Submission.ON_HOLD:
        created = row.created or datetime.datetime.now()
