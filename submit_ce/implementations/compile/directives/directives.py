...
class DirectiveManager:
    ...
    def __init__(self, root_dir: str=None, src_dir: str | None = None,
                 zzrm_content: str = None, preflight_content: str = None):
        ...
        self.zzrm_content = zzrm_content
        self.preflight_content = preflight_content

    def load_preflight_data(
                self, 
                preflight_file_arg: str | None = None, 
                preflight_content: str = None,
    ):
        """Load preflight data and returns a hierarchy."""
        # Assuming the preflight module returns a hierarchy when loading data
        if self.preflight_content != None:
            try:
                data = json.loads(self.preflight_content)
                return data
            except (json.JSONDecodeError, ZZRMInvalidFormatError) as e:
                error_message = f"Error parsing zzrm_content build directives file: {e}"
    ...
    def process_directives(self, dest_format: str = "json"):
        ...
        data = None
        if self.zzrm_content != None:
            if self.zzrm_content == "":
                data = {}
            else:
                try:
                    data = json.loads(self.zzrm_content)
                except (json.JSONDecodeError, ZZRMInvalidFormatError) as e:
                    error_message = f"Error parsing zzrm_content build directives file: {e}"
                    raise ValueError(error_message)
        else:
