...
def test_create_submission(authorized_client):
    ...
    response = client.get(f'/{upload_id}/file_upload')
    assert response.status_code in [ status.FOUND, status.SEE_OTHER ]

    response = client.get(f'/{upload_id}/final_preview')
    assert response.status_code in [ status.FOUND, status.SEE_OTHER ]

    response = client.get(f'/{upload_id}/add_optional_metadata')
    assert response.status_code in [ status.FOUND, status.SEE_OTHER ]
    ...
    response = client.get(next_page.path)
    assert b'I am an author of this paper' in response.data
    token = _parse_csrf_token(response)
