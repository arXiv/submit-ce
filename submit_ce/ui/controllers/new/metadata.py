...
def _commands(form: MetadataForm, submission: Submission,
    ...
    if form.title.data is not None and submission.metadata and (form.title.data != submission.metadata.title or not form.title.data):
        command = SetTitle(title=form.title.data, creator=creator,
                           client=client)
        valid.append(validate_command(form, command, submission, 'title'))
        commands.append(command)

    if form.abstract.data is not None and submission.metadata and (form.abstract.data != submission.metadata.abstract or not form.abstract.data):
    ...
    if form.comments.data and submission.metadata and form.comments.data != submission.metadata.comments:
    ...
    if value is not None and submission.metadata and (value != submission.metadata.authors_display or not value):
    ...
    if form.msc_class.data and submission.metadata and form.msc_class.data != submission.metadata.msc_class:
    ...
    if form.acm_class.data and submission.metadata and form.acm_class.data != submission.metadata.acm_class:
    ...
    if form.report_num.data and submission.metadata and form.report_num.data != submission.metadata.report_num:
        command = SetReportNumber(report_num=form.report_num.data,
                                  creator=creator, client=client)
        valid.append(validate_command(form, command, submission, 'report_num'))
        commands.append(command)

    if form.journal_ref.data and submission.metadata and form.journal_ref.data != submission.metadata.journal_ref:
    ...
    if form.doi.data and submission.metadata and form.doi.data != submission.metadata.doi:
