...
class StartedAlterExising(BaseModel):
    ...
    Starts a submission.
    """ # noqa: E501
    submission_type: StrictStr
    paperid: StrictStr
    __properties: ClassVar[List[str]] = ["submission_type", "paperid"]

    @field_validator('submission_type')
    def submission_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['replacement', 'withdrawal', 'cross', 'jref']):
            raise ValueError("must be one of enum values ('replacement', 'withdrawal', 'cross', 'jref')")
        return value
